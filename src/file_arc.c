...
/* everything is cartesian after latmat mult */
    ARR3SET(x, core->x);
/* use latmat of new lattice parameters to cope with rotated cells */
    matrix_lattice_from_pbc(latmat, model->pbc, model->periodic);
    vecmat(latmat, x);
...
g_free(tamp);
/* Nb. title of calculation is given in the first 64 characters BUT it can be empty*/
...
/* increment/reset end counter according to input line <- strstr at the begining of the line */
  if (g_ascii_strncasecmp("end", line, 3) == 0)
    end_count++;
  else
    end_count = 0;
...
/* cell dimension search */
  if (g_ascii_strncasecmp("pbc", line, 3) == 0)
    {
    data->pbc[5]=-1.;
    sscanf(line, "PBC %lf %lf %lf %lf %lf %lf %*s", &(data->pbc[0]), &(data->pbc[1]), 
		    &(data->pbc[2]), &(data->pbc[3]), &(data->pbc[4]), &(data->pbc[5]));
#if DEBUG_ARC
fprintf(stdout,"#DBG: LINE:  %lf %lf %lf %lf %lf %lf\n", data->pbc[0], data->pbc[1], 
		             data->pbc[2], data->pbc[3], data->pbc[4], data->pbc[5]);
#endif
    if((data->pbc[5])>=0)
      { /* 6 tokens -> PCB=ON with or without implicit 2D */
	data->pbc[3] *= D2R;
	data->pbc[4] *= D2R;
	data->pbc[5] *= D2R;
	if((data->pbc[2])==0)
	  ...
	  data->periodic = 2;
        ...
        { /* less than 6 tokens -> PCB=2D */
	data->pbc[5] = D2R*(data->pbc[2]);
	data->pbc[2] = 1.0;
	data->pbc[3] = 0.5*G_PI;
	data->pbc[4] = 0.5*G_PI;
        }
      matrix_lattice_init(data); /* initialize matrix */
#if DEBUG_ARC
...
#endif
      data->fractional = TRUE; /* set fractional */
      g_free(line);
      continue;
    }

/* each coord line consists of
 * [name], x, y, z, type, [seq. nb], pot. type, elem symbol, charge [, atom nb] */
    c_symb[0]='-';c_symb[1]='\0';
    sscanf(line,"%*s %lf %lf %lf %s %*s %s %s %lf %*s", 
		  &x, &y, &z, &(c_type[0]), &(c_ptype[0]), &(c_symb[0]), &charge);
#if DEBUG_ARC
...
for (;;)
  {
  fp_pos = ftell(fp); /* get position of line before reading next */
  line = file_read_line(fp);
  if(!line)
    break;
